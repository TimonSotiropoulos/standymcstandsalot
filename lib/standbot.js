...
StandBot.prototype._onOpen = function() {
    ...
    console.log("Current Time while starting the Application!");
    var currentTime = new moment.tz("Australia/Melbourne");
...
StandBot.prototype._waitForReady = function(self) {
    ...
    console.log("Creating a New Check MEssages Call - _WAIT FOR READY");
    self.timeoutCallback = setTimeout(self._checkSendMessages.bind(null, self), 10000);
}
...
StandBot.prototype._startStandup = function(self) {
    ...
    self.welcome = true;
    self.runningStandup = true;
    console.log("Creating a New Check MEssages Call - START STANDUP");
    self.timeoutCallback = setTimeout(self._checkSendMessages.bind(null, self), STANDUP_REMINDER_TIME);
}
...
StandBot.prototype._checkSendMessages = function(self) {
    ...
    if (usersToStillWork.length > 0 && self.reminderCounter < TOTAL_REMINDERS) {
        ...
        console.log("Creating a New Check MEssages Call - CHECK SEND MESSAGES");
        self.timeoutCallback = setTimeout(self._checkSendMessages.bind(null, self), STANDUP_REMINDER_TIME);
    } else {
        self._endStandup(self);
    }
...
StandBot.prototype._setupNextStandup = function(self) {
    ...
    var timeToNextStandup = 10000;

    // Schedule the next start up
    console.log("Creating a New Check MEssages Call - SETUP NEXT STANDUP");
    self.timeoutCallback = setTimeout(self._startStandup.bind(null, self), timeToNextStandup);

}

StandBot.prototype._calcTimeTillNextStandup = function() {
    // Check if tomorrow is friday
    var nextDay = new moment.tz("Australia/Melbourne").add(1, 'd');
    var nextStandup;
    // If the next day is Saturday, then we need to skip to monday
    if (nextDay.day() === 6) {
        nextStandup = new moment.tz("10", "hh", "Australia/Melbourne").add(72, "h");
    } else {
        nextStandup = new moment.tz("10", "hh", "Australia/Melbourne").add(24, "h");
    }

    console.log("Next Day");
    console.log(nextDay.format());

    console.log("Next Standup");
    console.log(nextStandup.format());
    ...
    console.log("Time till next standup");
